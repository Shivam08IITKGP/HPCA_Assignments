#!/usr/bin/env python3
# prob_4.py — Run from inside /home/shivam/Projects/cacti-master/
# Usage: python3 prob_4.py > prob_4.log
...
CACTI      = "./cacti"
BASE_CFG   = "./cache.cfg"
CFG_DIR    = "./prob4_configs"
PLOT_FILE  = "./prob_4_fig29.png"
...
def parse_energy(output):
    ...
    m = re.search(r'Total dynamic read energy per access \(nJ\):\s*([\d.]+)', output)
    return float(m.group(1)) if m else None

# ── Run all 20 configurations ──────────────────────────────────────────────
print("=" * 65)
print("Problem 4: CACTI — Energy per Read vs Cache Size (Fig 2.9)")
print("=" * 65)
print()
...
for assoc in ASSOCS:
    for i, size in enumerate(SIZES):
        ...
        e = parse_energy(output)

        if e:
            results[assoc][i] = e
            print(f"    Read energy : {e:.6f} nJ")
        else:
            print(f"    WARNING: Could not parse energy. Raw:\n{output[:400]}")
            results[assoc][i] = 0.0
        print()

# ── Summary table ──────────────────────────────────────────────────────────
print("=" * 65)
print("SUMMARY TABLE — Total Dynamic Read Energy per Access (nJ)")
print("=" * 65)
...
for a in ASSOCS:
    ...
    for i in range(len(SIZES)):
        v = results[a][i]
        row += f"{v:>10.5f}" if v else f"{'N/A':>10}"
    print(row)
print("=" * 65)
print()
...
ax.set_ylabel("Read energy per access (nJ)", fontsize=12)
ax.set_title("Cache Read Energy vs Cache Size\n"
             "(Fig 2.9 — CACTI 7.0, 90nm, 64B blocks)", fontsize=12)
ax.set_xticks(x)
