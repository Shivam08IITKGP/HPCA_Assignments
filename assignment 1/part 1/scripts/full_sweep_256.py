...
test_binary = os.path.join(project_base, "benchmarks/matrix_multiply_256x256")
sweep_output = os.path.join(project_base, "results/full_sweep_256")
results_file = os.path.join(sweep_output, "full_sweep_results_256.csv")
...
def build_benchmark():
    ...
    build_cmd = [
        ...
        "-O2", "-static", "-DMATRIX_SIZE=256",
        src_file, "-o", test_binary
    ]
    
    try:
        subprocess.run(build_cmd, check=True)
        print(f"Compiled benchmark for 256x256 matrix: {test_binary}")
    except subprocess.CalledProcessError:
        print(f"Failed to compile benchmark for 256x256 matrix")
        exit(1)
...
if __name__ == "__main__":
    ...
    print(f"Starting Full Sweep for 256x256 on {parallel_workers} cores ({len(all_configurations)} total configs)...")
