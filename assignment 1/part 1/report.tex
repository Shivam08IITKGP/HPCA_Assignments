...
The matrix multiplication benchmark uses a single source file with conditional compilation:
...
\paragraph{Analysis:}
The 256x256 matrix shows the highest sensitivity to L2 scaling. There is a sharp drop in execution time when the L2 size increases from 128kB to 256kB. So this identifies the "critical working set" for this matrix multiplication kernel; at 128kB, the L2 is thrashing, but at 256kB and above, it can retain enough tiles to significantly lower the Average Memory Access Time (AMAT).
...
\begin{figure}[h!]
    ...
    \caption{Execution Time Heatmaps for 128x128 (left) and 256x256 (right) matrices. The 64x64 matrix heatmap is omitted as it shows negligible variation across all cache configurations due to its small working set fitting entirely within even the smallest L1 cache tested.}
    \label{fig:heatmaps}
\end{figure}
